...
export const SPLIT_ORDER: GymSplit[] = ["PUSH", "PULL", "LEGS"];
...
export function saveWorkout(session: WorkoutSession) {
  ...
  const index = workouts.findIndex((item) => item.id === session.id);
...
export function getLastSessionData(name: string): PreviousExerciseSnapshot | null {
  const latest = getExerciseHistory(name).at(-1);
  if (!latest) return null;

  const topSet = getTopSet(latest.sets);
  return {
    date: latest.date,
    weight: topSet?.weight ?? 0,
    reps: topSet?.reps ?? 0,
    sets: latest.sets.filter((set) => set.completed).length,
...
export function getExercisePersonalBest(name: string): PreviousExerciseSnapshot | null {
  ...
  history.forEach((entry) => {
    ...
    const candidate: PreviousExerciseSnapshot = {
      date: entry.date,
...
export function createWorkoutSession(split: GymSplit, scheduledSplit = split): WorkoutSession {
  return {
    id: `gym_${Date.now()}`,
    date: getIsoDate(),
    split,
    scheduledSplit,
    manualOverride: split !== scheduledSplit,
    startTime: Date.now(),
    endTime: null,
    exercises: ROUTINES[split].map((name, index) => ({
...
export function buildWorkoutSummary(session: WorkoutSession, energyAfter: number): WorkoutSession {
  ...
  const exercises = session.exercises.map((exercise) => {
    ...
    const bestPastVolume = Math.max(0, ...priorHistory.map((entry) => entry.volume));
...
export function getExerciseHistory(name: string): ExerciseHistoryEntry[] {
  return getWorkouts()
    ...
    .flatMap((workout) =>
      workout.exercises
        .filter((exercise) => exercise.name === name && getExerciseVolume(exercise.sets) > 0)
        .map((exercise) => ({
...
export function getExerciseProgress(name: string, currentSets: GymSet[]): ExerciseProgress | null {
  ...
  if (currentTopSet.reps > lastSession.reps) {
    return {
      tone: "up",
      label: `↑ +${currentTopSet.reps - lastSession.reps} rep${currentTopSet.reps - lastSession.reps === 1 ? "" : "s"}`,
      isNewPr: false,
    };
  }
  ...
  if (currentTopSet.reps < lastSession.reps) {
    return {
      tone: "down",
      label: `↓ ${currentTopSet.reps - lastSession.reps} reps`,
      isNewPr: false,
    };
  }
...
export function getGymOverview(): GymOverview {
  ...
  if (dates.length > 0) {
    ...
    if (latestDiff <= 1) {
      ...
      const historyWindow = getDatesBetween(new Date(dates[0]), latestLoggedDate).reverse();
